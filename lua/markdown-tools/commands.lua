...
function M.insert_code_block(opts)
  ...
  vim.ui.input({ prompt = "Language (leave empty for no language): " }, function(lang)
    ...
    if mode:match("^[vV]") or opts.range == 2 then -- Added opts.range check
      ...
      local start_line_content = vim.api.nvim_buf_get_lines(0, start_lnum_1based - 1, start_lnum_1based, false)[1] or ""
...
function M.insert_link(opts)
  ...
  vim.ui.input({ prompt = "Enter URL: " }, function(url)
    ...
    if not is_visual or link_text == "" then
      ...
      local start_line_content = vim.api.nvim_buf_get_lines(0, start_lnum_zero, start_lnum_zero + 1, false)[1] or ""
      ...
      vim.api.nvim_buf_set_text(0, start_lnum_zero, start_col_byte, end_lnum_zero, end_col_byte, { markdown_link })
