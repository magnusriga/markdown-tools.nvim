...
local function safe_call_frontmatter_fn(fn, opts, field_name, expected_type)
	...
	if expected_type ~= "any" then
		if expected_type == "string" and type(result) ~= "string" then
			vim.notify(string.format("%s function must return a string or nil, got %s", field_name, type(result)), vim.log.levels.WARN)
			return nil
		elseif expected_type == "table" and type(result) ~= "table" then
			vim.notify(string.format("%s function must return a table or nil, got %s", field_name, type(result)), vim.log.levels.WARN)
			return nil
		end
	end
...
function M.handle_template_selection(template_path, opts)
	...
	vim.schedule(function()
		...
		vim.ui.input({ prompt = "Enter new file name: " }, function(input_file_name)
			...
			local processed_lines = process_template_content(template_content, new_file_path, frontmatter_values, timestamp, opts)
